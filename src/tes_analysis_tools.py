...
def optimal_filter_freq(pulse, model, noise, dt, maxfreq, showplot, verbose):
    ...
    ind = np.where(freq[0:np.int(dp/2)] < maxfreq)
...
def spectrum_density(wave, dt, Rf, Mf, Min, showplot, verbose):
    ...
    if verbose == True:
        ...
        print("...")
        print("[k = N/2 - 2]: freq = ", freq[np.int(N/2) - 2], ", F[k] = ", X[np.int(N/2) - 2])
        print("[k = N/2 - 1]: freq = ", freq[np.int(N/2) - 1], ", F[k] = ", X[np.int(N/2) - 1])
        print("")
        
        print("*** Nyquist freq ***")
        print("[k = N/2] (nyquist freq) : ", freq[np.int(N/2)], ", F[k] = ", X[np.int(N/2)])
        print("")
    ...
    # -----------------------------------
    # |X(k)| [V]
    freq_ = freq[0 : np.int(N/2)]  # 周波数範囲:片側
    X_abs = np.abs(X[0 : np.int(N/2)])
    ...
    PS[1 : np.int(N/2)] = PS[1 : np.int(N/2)] * 2.0
